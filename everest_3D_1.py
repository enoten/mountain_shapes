...
WIRE_COLOR = "#2EC3BD"  # Cyan/turquoise color

# Create glow effect by drawing multiple wireframes with decreasing opacity
# Outer glow layers (behind main wireframe)
glow_layers = [
    {"linewidth": 6.0, "alpha": 0.15, "offset": 0},  # 3x thicker (was 2.0)
    {"linewidth": 4.5, "alpha": 0.25, "offset": 0},  # 3x thicker (was 1.5)
    {"linewidth": 3.0, "alpha": 0.35, "offset": 0},  # 3x thicker (was 1.0)
]
...
ax.plot_wireframe(
    ...
    cstride=2,
    color=WIRE_COLOR,
    linewidth=1.8,  # 3x thicker (was 0.6)
    alpha=0.9,
)
...
# Peak marker
box_size = 200  # Size of the transparent square around peak

# Draw a transparent square outline around the peak point
square_corners_x = [
    peak_x - box_size / 2,
    peak_x + box_size / 2,
    peak_x + box_size / 2,
    peak_x - box_size / 2,
    peak_x - box_size / 2,  # Close the square
]
square_corners_y = [
    peak_y - box_size / 2,
    peak_y - box_size / 2,
    peak_y + box_size / 2,
    peak_y + box_size / 2,
    peak_y - box_size / 2,  # Close the square
]
square_corners_z = [peak_z] * 5  # All corners at peak elevation
...
# Add glow effect to base grid too
for glow in [{"linewidth": 2.4, "alpha": 0.1}, {"linewidth": 1.5, "alpha": 0.15}]:  # 3x thicker
    ax.plot_wireframe(
        X,
        Y,
        base_z,
...
ax.plot_wireframe(
    ...
    cstride=5,
    color=WIRE_COLOR,
    linewidth=0.9,  # 3x thicker (was 0.3)
    alpha=0.2,
)
...
# Initial view angle for a nice 3D silhouette
initial_elev = 30
initial_azim = -50
...
# Animation function to rotate around z-axis (counter-clockwise)
def animate(frame):
    """Rotate the view around z-axis (azimuth angle) - counter-clockwise"""
    # Rotate 360 degrees counter-clockwise over the animation
    # frame goes from 0 to num_frames, so azim decreases from initial_azim to initial_azim - 360
    azim_angle = initial_azim - (frame * 360 / num_frames)
    ax.view_init(elev=initial_elev, azim=azim_angle)
    return []

# Create animation
num_frames = 360  # Number of frames for full rotation (1 degree per frame)
interval = 20  # Delay between frames in milliseconds (20ms = 50 fps)

print("Starting animation... Rotating around z-axis. Close the window to stop.")
ani = FuncAnimation(fig, animate, frames=num_frames, interval=interval, blit=False, repeat=True)
